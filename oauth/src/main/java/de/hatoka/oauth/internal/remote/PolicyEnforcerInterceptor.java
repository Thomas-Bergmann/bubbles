...
public class PolicyEnforcerInterceptor implements HandlerInterceptor
{
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception
    {
        ...
        else if (uri.getPath().startsWith(UserTokenController.PATH_ROOT))
        {
            // allow retrieving tokens
            logger.trace("Client allowed to access uri '{}'' with method '{}'", uri.getPath(), request.getMethod());
            return true;
        }
        else if (bearer.isPresent() && tokenUtils.isTokenValid(bearer.get(), TokenUsage.access))
        {
            return true;
        }
